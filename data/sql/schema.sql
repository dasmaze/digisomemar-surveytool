...
CREATE TABLE question (id BIGINT AUTO_INCREMENT, text VARCHAR(255) NOT NULL, discription LONGTEXT, multichoice TINYINT(1), survey_id BIGINT, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, INDEX survey_id_idx (survey_id), PRIMARY KEY(id)) ENGINE = INNODB;
CREATE TABLE survey (id BIGINT AUTO_INCREMENT, title VARCHAR(255) NOT NULL, discription LONGTEXT, public_id VARCHAR(16) NOT NULL, limit_ip TINYINT(1), limit_location TINYINT(1), limit_location_lat FLOAT(18, 2), limit_location_long FLOAT(18, 2), limit_endtime DATETIME, user_id BIGINT, INDEX user_id_idx (user_id), PRIMARY KEY(id)) ENGINE = INNODB;
